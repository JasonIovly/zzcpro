<template>
  <div class="smeditor" id="smeditor">
    ...
    <div
      ...
      <h1>asdf</h1>
      <vue-fabric ref="canvas" :width="200" :height="200" style="background-color: red"></vue-fabric>
    </div>
    ...
    <button @click="hello()">asdfasdf</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    hello(){
      this.$refs.canvas.createTextbox('斯诺伐克两三', { top: 0, left: 0 });
    },
    // 回车事件
    kenter (e) {
      e.stopPropagation()
 ...
 //      alert(editorElement().innerHTML)

      window.localStorage.setItem('smeditorPreview',  '')
